...
def get_value_policy_reward(
    gfn_state: tuple, unobserved_cliques: list, clique_potentials: list, K: int
):
    """
    ...
    for c_ind in range(num_cliques):
        if (
            ...
            reward += clique_potentials[c_ind](
                gfn_state[1][np.array(sorted(list(unobserved_cliques[c_ind])))]
            )
...
if __name__ == "__main__":
    ...
    clique_potentials = [lambda c: 0.2, lambda c: -2]
    gfn_state = (
    ...
    new_gfn_state, new_unobserved_cliques, reward = get_value_policy_reward(
        gfn_state, unobserved_cliques, clique_potentials, K
    )
    assert reward == 0.2
    ...
    new_gfn_state, new_unobserved_cliques, reward = get_value_policy_reward(
        new_gfn_state, new_unobserved_cliques, clique_potentials, K
    )
    assert reward == 0.0
