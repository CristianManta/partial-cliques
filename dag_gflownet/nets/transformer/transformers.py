...
from dag_gflownet.nets.transformer.attention import LinearMultiHeadAttention
...
class DenseBlock(hk.Module):
    ...
    def __call__(self, inputs):
        ...
        hiddens = hk.Linear(self.widening_factor * self.output_size, w_init=w_init)(
            inputs
        )
...
class TransformerBlock(hk.Module):
    ...
    def __call__(self, hiddens, inputs):
        ...
        inputs_embedding = hk.Linear(
            self.embedding_size, w_init=w_init, name="linear_1"
        )(inputs)
        h_norm = hk.LayerNorm(
            axis=-1, create_scale=True, create_offset=True, name="layernorm_1"
        ...
        inputs_embedding = hk.Linear(
            self.embedding_size, w_init=w_init, name="linear_2"
        )(inputs)
        h_norm = hk.LayerNorm(
            axis=-1, create_scale=True, create_offset=True, name="layernorm_2"
        )(jnp.concatenate((inputs_embedding, hiddens), axis=-1))
        h_dense = DenseBlock(
